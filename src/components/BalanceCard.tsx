...
export const BalanceCard: React.FC<BalanceCardProps> = ({
  ...
  return (
    <div className="glass-panel p-4 sm:p-5 rounded-2xl border border-slate-200 dark:border-slate-800/80 flex flex-col justify-between gap-3.5 sm:gap-4 transition-colors duration-300">
      {/* Top Header Bar */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-cyan-500/10 border border-cyan-300 dark:border-cyan-500/30 flex items-center justify-center text-cyan-700 dark:text-cyan-400 shadow-sm">
            <Wallet className="w-4 h-4" />
          </div>
          <div>
            <h3 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-slate-100 tracking-tight">XM Margin Overview</h3>
            <p className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400">Live MetaTrader Account Equity</p>
          </div>
        </div>
        ...
        <button
          ...
          disabled={isLoading}
          className="p-1.5 sm:p-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 transition-all border border-slate-300 dark:border-slate-800 disabled:opacity-50 shadow-sm"
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3 bg-slate-50 dark:bg-slate-900/70 p-3 sm:p-3.5 rounded-xl border border-slate-200 dark:border-slate-800/80">
        <div>
          <span className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 font-semibold">Free Margin ({balance?.currency || 'USD'})</span>
          <div className="text-sm sm:text-lg font-bold text-slate-900 dark:text-slate-100 font-mono mt-0.5 truncate">
            ${availMargin.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </div>
        </div>

        <div>
          <span className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 font-semibold">Purchasing Power ({leverage}x)</span>
          <div className="text-sm sm:text-lg font-bold text-cyan-700 dark:text-cyan-400 font-mono mt-0.5 truncate">
      ...
      {totalBalance > 0 && (
        <div className="space-y-1">
          <div className="flex justify-between text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 font-semibold">
      ...
      <div className="pt-2 border-t border-slate-200 dark:border-slate-800/40 flex flex-col gap-2">
        <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 font-semibold">
          ...
          <div className="flex items-center gap-1.5 font-mono">
            <span className="text-cyan-700 dark:text-cyan-400 font-bold text-xs">{allocPct}%</span>
          ...
          {[25, 50, 75, 90, 100].map((pct) => (
            <button
              ...
              className={`py-1.5 px-1 text-xs font-bold rounded-xl transition-all shadow-sm active:scale-95 flex items-center justify-center ${
                allocPct === pct
                  ? 'bg-cyan-600 dark:bg-cyan-500 text-white dark:text-slate-950 shadow-sm border border-cyan-600 dark:border-cyan-400 font-extrabold'
                  : 'bg-white hover:bg-slate-100 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-800'
              }`}
